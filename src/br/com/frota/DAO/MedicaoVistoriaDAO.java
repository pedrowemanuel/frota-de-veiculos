...
public class MedicaoVistoriaDAO extends ConexaoDB {
	...
	public MedicaoVistoria selectMedicaoVistoria(int id) {
		...
		try (PreparedStatement preparedStatement = prapararSQL(SELECT_BY_ID)) {
			...
			while (rs.next()) {
				...
				int idVistoriaPneu = rs.getInt("id_vistoria_pneu");
				entidade = new MedicaoVistoria(id, raio, perfil, largura, altura, indiceCarga, indiceVelocidade,
						new MarcaPneu(idMarcaPneu), new VistoriaPneu(idVistoriaPneu));
			}
		} catch (SQLException e) {
			printSQLException(e);
		} catch (ClassNotFoundException e) {
	...
	public List<MedicaoVistoria> selectAllMedicaoVistorias() {
		...
		try (PreparedStatement preparedStatement = prapararSQL(SELECT_ALL)) {
			...
			while (rs.next()) {
				...
				int idMarcaPneu = rs.getInt("id_marca_pneu");
				int idVistoriaPneu = rs.getInt("id_vistoria_pneu");
				entidades.add(new MedicaoVistoria(id, raio, perfil, largura, altura, indiceCarga, indiceVelocidade,
						new MarcaPneu(idMarcaPneu), new VistoriaPneu(idVistoriaPneu)));
			}
		} catch (SQLException e) {
			printSQLException(e);
	...
	public boolean updateMedicaoVistoria(MedicaoVistoria entidade) throws SQLException {
		try (PreparedStatement statement = prapararSQL(UPDATE)) {
			...
			statement.setInt(8, entidade.getVistoriaPneu().getId());
