    ...
    public Pneu selectPneu(int id) {
        ...
        try (PreparedStatement preparedStatement = prapararSQL(SELECT_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                entidade = new Pneu(id, raio, perfil, largura, altura, indiceCarga, indiceVelocidade, new MarcaPneu(idMarcaPneu));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
    ...
    public List<Pneu> selectAllPneus() {
        ...
        try (PreparedStatement preparedStatement = prapararSQL(SELECT_ALL)) {
            ...
            while (rs.next()) {
                ...
                String indiceVelocidade = rs.getString("indice_velocidade");
                int idMarcaPneu = rs.getString("id_marca_pneu");
                entidades.add(new Pneu(id, raio, perfil, largura, altura, indiceCarga, indiceVelocidade, new MarcaPneu(idMarcaPneu)));
            }
        } catch (SQLException e) {
            printSQLException(e);
        } catch (ClassNotFoundException e) {
